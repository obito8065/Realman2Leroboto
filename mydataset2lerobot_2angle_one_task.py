...
# observation.state: 6 joints + 6 joints + 6 (xyz+angle) + 6 (xyz+angle) +18 (hands)+ 18(hands) = 60
# observation.action: 6 joints + 6 joints + 6 (xyz+angle) + 6 (xyz+angle)
...
def load_local_dataset(hdf5_path: str) -> tuple:
    ...
    with h5py.File(hdf5_path,"r") as hdf_file:
        ...
        # 读取图像数据
        images = {
            "cam_head": [decompress_image(frame) for frame in hdf_file['observations/images/cam_head']],
        }
        # 拆分图像数据为左右两张图片
        cam_head_left = [frame[:, :1280, :] for frame in images["cam_head"]]
        cam_head_right = [frame[:, 1280:, :] for frame in images["cam_head"]]
        ...
        save_video(cam_head_left, cam_head_left_path, fps=20, feature_key="observation.images.cam_head_left")
        save_video(cam_head_right, cam_head_right_path, fps=20, feature_key="observation.images.cam_head_right")
...
class AgiBotDataset(LeRobotDataset):
    ...
    def save_episode(
        ...
        #     raise ValueError(f" {task_index} is none.")
        ...
        for key in self.meta.video_keys:
            ...
            print(f"Generated video path: {videos[key]}")
...
def main(hdf5_dir: str, output_dir: str, task_name: str, fps: int = 20):
    """
    将单个 HDF5 文件转换为 LeRobot 数据集格式。

    :param hdf5_path: 输入的 HDF5 文件路径
    ...
    if not hdf5_files:
        raise ValueError(f"No HDF5 files found in directory: {hdf5_dir}")
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--hdf5_dir", type=str, default="/home/jdtech/Documents/dataset_convert/",required=False, help="Realman 数据集的 HDF5 文件路径")
    # parser.add_argument("--output_dir", type=str, default="/home/jdtech/Documents/dataset_convert/",required=False, help="输出目录路径")
    # parser.add_argument("--task_name", type=str, default="take_cup",required=False, help="任务名称")
...
# python /home/jdtech/Documents/dataset_convert/mydataset2lerobot_2angle_one_task.py --hdf5_dir ./lerobot_cup_test --output_dir ./take_cup_test --task_name take_cup --fps 20